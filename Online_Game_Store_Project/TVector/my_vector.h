...
public:
    ...
    inline const State* states() const noexcept; // для отладки
...
void TVector<T>::push_back(const T& value) {
    if (_size != 0) {
        ...
        if (last_busy_pos + 1 < _capacity && (_states[last_busy_pos + 1] != busy)) {
...
void TVector<T>::insert(size_t pos, size_t count, const T& value) {
    ...
    for (size_t i = _size + _deleted + count - 1; i >= insert_pos + count; i--) {
...
void TVector<T>::reallocate_for_deleted() {
    ...
    if (_deleted >= DELETED_THRESHOLD * (_size + _deleted) || _deleted >= MAX_DELETED_COUNT) {
        size_t new_capacity = (_size / STEP_OF_CAPACITY + 1) * STEP_OF_CAPACITY;
        T* new_data = new T[new_capacity];
        State* new_states = new State[new_capacity];
